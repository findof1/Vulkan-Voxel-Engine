...
private:
    ...
    // std::vector<uint32_t> indices = cubeIndices;
    ...
    void createIndexBuffer(std::vector<uint32_t> inputIndices, int targetBuffer)
    {
        ...
        createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, indexBuffers[targetBuffer], indexBufferMemory[targetBuffer]);

        copyBuffer(stagingBuffer, indexBuffers[targetBuffer], bufferSize);

        vkDestroyBuffer(device, stagingBuffer, nullptr);
        vkFreeMemory(device, stagingBufferMemory, nullptr);
    }

    void createVertexBuffer(std::vector<Vertex> verts, int targetBuffer)
    {
        ...
        createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, vertexBuffers[targetBuffer], vertexBufferMemory[targetBuffer]);

        copyBuffer(stagingBuffer, vertexBuffers[targetBuffer], bufferSize);
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
    {
        ...
        VkBuffer vertexBuffersArray[] = {vertexBuffers[0]};
    ...
    void updateUniformBuffer(uint32_t currentImage, glm::vec3 position)
    {
